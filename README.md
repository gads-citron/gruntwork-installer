...
```
curl -LsS https://raw.githubusercontent.com/gads-citron/gruntwork-installer/v0.0.38/bootstrap-gruntwork-installer.sh | bash /dev/stdin --version v0.0.38
```

Notice the `--version` parameter at the end where you specify which version of `gruntwork-install` to install. See the
[releases](https://github.com/gads-citron/gruntwork-installer/releases) page for all available versions.
...
`--tag`                     | Yes      | The version of the `--repo` to install from.<br>Follows the syntax described at [Tag Constraint Expressions](https://github.com/gads-citron/fetch#tag-constraint-expressions). This value is exposed to module install scripts as GRUNTWORK_INSTALL_TAG.
...
module](https://github.com/gads-citron/terraform-aws-ecs/tree/main/modules/ecs-scripts) from the [terraform-aws-ecs
repo](https://github.com/gads-citron/terraform-aws-ecs), version `v0.0.1`:

```
gruntwork-install --module-name 'ecs-scripts' --repo 'https://github.com/gads-citron/terraform-aws-ecs' --tag 'v0.0.1'
```

##### Example 2: Download and Install a Script Module with Parameters

Install the [fail2ban
module](https://github.com/gads-citron/terraform-aws-security/tree/main/modules/fail2ban) from the [terraform-aws-security
repo](https://github.com/gads-citron/terraform-aws-security), passing two custom parameters to it:
...
repo](https://github.com/gads-citron/gruntkms):

```
gruntwork-install --binary-name 'gruntkms' --repo 'https://github.com/gads-citron/gruntkms' --tag 'v0.0.1'
```

Note that the [v0.0.1 release of the gruntkms repo](https://github.com/gads-citron/gruntkms/releases/tag/v0.0.1) has
  ...
  "provisioners": [
    {
      "type": "shell",
      "inline":
        "curl -Ls https://raw.githubusercontent.com/gads-citron/gruntwork-installer/v0.0.38/bootstrap-gruntwork-installer.sh | bash /dev/stdin --version v0.0.16"
    },
    {
      "type": "shell",
      "inline": [
        "gruntwork-install --module-name 'ecs-scripts' --repo 'https://github.com/gads-citron/terraform-aws-ecs' --tag 'v0.0.1'",
        "gruntwork-install --module-name 'fail2ban' --repo 'https://github.com/gads-citron/terraform-aws-security' -module-param 'ban-time=3600'",
        "gruntwork-install --binary-name 'gruntkms' --repo 'https://github.com/gads-citron/gruntkms' --tag 'v0.0.1'"
      ],
...
1. It uses [fetch](https://github.com/gads-citron/fetch) to download the specified version of the scripts or binary from
...
module](https://github.com/gads-citron/terraform-aws-ecs/tree/main/modules/ecs-scripts) as follows:

```
gruntwork-install --module-name 'ecs-scripts' --repo 'https://github.com/gads-citron/terraform-aws-ecs' --tag 'v0.0.1'
```

In https://github.com/gads-citron/module-ecs, we download the contents of `/modules/ecs-scripts` and run
